...
/**
 * @summary
 * A class that provides functionality to call the ServiceObjects AVCA2 (Address Validation - Canada) SOAP service's ValidateCanadianMunicipalityProvince endpoint,
...
class ValidateCanadianMunicipalityProvinceSoap {
    /**
     * @summary
     * Initializes a new instance of the ValidateCanadianMunicipalityProvinceSoap class with the provided input parameters,
     ...
     */
    constructor(Municipality, Province, PostalCode, LicenseKey, isLive = true, timeoutSeconds = 15) {
        this.args = {
            Municipality,
            Province,
            PostalCode,
    ...
    /**
     * @summary
     * Asynchronously calls the ValidateCanadianMunicipalityProvince SOAP endpoint, attempting the primary endpoint
     * first and falling back to the backup if the response is invalid (Error.TypeCode == '3') in live mode
     * or if the primary call fails.
     * @returns {Promise<Object>} A promise that resolves to an object containing validated municipality and province details or an error.
     ...
     * @param {Object} args - The arguments to pass to the ValidateCanadianMunicipalityProvince method.
    ...
    _callSoap(wsdlUrl, args) {
        return new Promise((resolve, reject) => {
            soap.createClient(wsdlUrl, { timeout: this.timeoutSeconds * 1000 }, (err, client) => {
                ...
                client.ValidateCanadianMunicipalityProvince(args, (err, result) => {
                    const response = result?.ValidateCanadianMunicipalityProvinceResult;
                    try {
                        if (!response) {
...
export { ValidateCanadianMunicipalityProvinceSoap };